...
import json, os, pathlib, shutil, sys, time, warnings
...
from ecopulse_ca.ingest.copernicus import (  # noqa: E402
    ADS_URL, CAMS_MAX_MONTHS_PER_REQUEST, DEPLOYABLE_LEADTIME_H, KG_M3_TO_UG_M3,
    assign_cells, extract_points, month_blocks, station_bbox, unzip_members)
...
print(f"CAMS PM2.5 | leadtime {DEPLOYABLE_LEADTIME_H}h | {len(blocks)} requests | area {area}",
      flush=True)

import cdsapi
c = cdsapi.Client(url=ADS_URL, key=(os.getenv("ADS_API_KEY") or "").strip(),
...
for n, block in enumerate(blocks, 1):
    ...
    req = {"variable": ["particulate_matter_2.5um"], "date": [f"{lo}/{hi}"],
           "time": ["00:00"], "leadtime_hour": [str(DEPLOYABLE_LEADTIME_H)],
           "type": ["forecast"], "area": area, "data_format": "netcdf"}
    try:
        ...
        for m in members:
            ...
            frames.append(extract_points(sq, {"pm2p5": "cams_pm25_forecast"}, cells, tdim,
    ...
    if n % 5 == 0 or n == len(blocks):
        print(f"  [{n}/{len(blocks)}] rows {sum(len(f) for f in frames):,} "
...
prov = {"dataset": "cams-global-atmospheric-composition-forecasts",
        ...
        "area_NWSE": area, "requests": len(blocks), "failed": failed,
        "rows": int(len(df)), "null_share": round(float(df.cams_pm25_forecast.isna().mean()), 4),
        "cell_collisions": {c.station_id: list(c.shares_cell_with) for c in (cells or [])
                            if c.shares_cell_with}}
