...
def main() -> int:
    pred = pd.read_csv(
        T / "t6_01_predictions_task_n.csv", dtype={"station_id": str}, parse_dates=["date"]
    )
    census = pd.read_csv(
    ...
    for fold, g in pred.groupby("fold"):
        ...
        rows.append(
            {
                "fold": fold,
                "instrument_grade": grade.get(fold, "?"),
                **m,
                "cams_rmse": mc["rmse"],
                "beats_cams": m["rmse"] < mc["rmse"],
                "obs_mean": float(g.pm25.mean()),
                "obs_sd": float(g.pm25.std()),
    ...
    bands = [
        ("clean (<= 1x WHO)", 0, WHO_24H),
        ("moderate (1-3x)", WHO_24H, 3 * WHO_24H),
        ("high (3-6x)", 3 * WHO_24H, 6 * WHO_24H),
        ("extreme (> 6x)", 6 * WHO_24H, np.inf),
    ]
    rows = []
    ...
    rows = [
        {"season": s, **metrics(g.pm25, g.lgbm), "obs_mean": float(g.pm25.mean())}
    ...
    print(
        bands_df[["band", "n", "share_of_rows", "rmse", "bias", "r2", "cams_rmse"]]
        .round(3)
    ...
    print("\n=== BY SEASON ===")
    print(
        pd.DataFrame(rows)[["season", "n", "rmse", "bias", "r2", "obs_mean"]]
        .round(3)
        .to_string(index=False)
    )

    # ---- what explains fold-level failure? ------------------------------------------
    print(f"\n=== WHAT PREDICTS A BAD FOLD? (Spearman, n = {len(per_fold)} folds) ===")
