...
print(
    f"feature table: {len(df):,} station-days, {df.station_id.nunique()} stations, "
...
from ecopulse_ca.models.feature_table import daily_target  # noqa: E402
...
for tier in TIERS:
    ...
    for fold in splits["leave_city_out"]:
        ...
        j = te.merge(
            sub[["station_id", "date", "pooled"]], on=["station_id", "date"], how="left"
        ).dropna(subset=["pooled"])
        if len(j) >= 30:
...
print("=" * 78)
piv = (
    res[res.model == "lightgbm"]
    .groupby(["tier"])
    .agg(rmse=("rmse", "mean"), mae=("mae", "mean"), r2=("r2", "mean"), rmse_sd=("rmse", "std"))
)
