...
from ecopulse_ca.ingest.copernicus import (  # noqa: E402
    CDS_URL, ERA5_MAX_MONTHS_PER_REQUEST, assign_cells, extract_points, month_blocks,
    station_bbox, unzip_members)
...
import cdsapi
c = cdsapi.Client(url=CDS_URL, key=(os.getenv("CDS_API_KEY") or "").strip(),
                  quiet=True, progress=False)

SINGLE_VARS = ["boundary_layer_height", "2m_temperature",
               "10m_u_component_of_wind", "10m_v_component_of_wind", "total_precipitation"]
RENAME = {"blh": "blh_m", "t2m": "t2m_k", "u10": "u10_ms", "v10": "v10_ms",
          "tp": "precip_prev_hour_m"}   # name encodes the (T-1h, T] convention
...
def run(dataset: str, label: str, variables: list[str], extra: dict, out_name: str) -> None:
    ...
    for n, block in enumerate(blocks, 1):
        ...
        req = {"product_type": ["reanalysis"], "variable": variables,
               "year": years, "month": months,
               "day": [f"{d:02d}" for d in range(1, 32)],
               "time": [f"{h:02d}:00" for h in range(24)],
               "area": area, "data_format": "netcdf",
               "download_format": "unarchived", **extra}
    ...
    (ROOT / f"data/interim/{out_name}_provenance.json").write_text(json.dumps({
        "dataset": dataset, "variables": variables, "extra": extra, "area_NWSE": area,
        "requests": len(blocks), "failed": failed, "rows": int(len(df)),
        "oracle_only": True,
        ...
        "cell_collisions": {x.station_id: list(x.shares_cell_with)
                            for x in (cells or []) if x.shares_cell_with},
    }, indent=2))
...
run("reanalysis-era5-pressure-levels", "ERA5 925 hPa", ["temperature"],
    {"pressure_level": ["925"]}, "era5_925")
