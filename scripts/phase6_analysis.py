...
va_lo, va_hi = pd.Timestamp(B["val"]["start"]).tz_localize(None), pd.Timestamp(B["val"]["end"]).tz_localize(None)
te_lo, te_hi = pd.Timestamp(B["test"]["start"]).tz_localize(None), pd.Timestamp(B["test"]["end"]).tz_localize(None)
...
best = {(r.tier, r.held_out_city): eval(r.params)
        for _, r in tuned[(tuned.task=="N")&(tuned.model=="lgbm_tuned")].drop_duplicates(
            ["tier","held_out_city"]).iterrows()}

panel = pd.read_parquet(ROOT/"data/interim/benchmark_panel.parquet"); panel.columns=[str(c) for c in panel.columns]
...
cams["station_id"]=cams.station_id.astype(str); cams["date"]=pd.to_datetime(cams.time).dt.normalize()
...
for fold in splits["leave_city_out"]:
    ...
    ens = []
    for seed in SEEDS:
        m = lgb.LGBMRegressor(n_estimators=800, learning_rate=lr, num_leaves=leaves,
                              min_child_samples=mcs, colsample_bytree=ff, subsample=0.8,
                              subsample_freq=1, random_state=seed, verbose=-1).fit(tr[feats], tr.pm25)
...
for fold, g in P.groupby("fold"):
    ...
    rows.append({"fold":fold,"n":r.n,"rmse_lgbm":regression_metrics(g.pm25,g.lgbm).rmse,
                 "rmse_cams":regression_metrics(g.pm25,g.pooled).rmse,
                 "dm":r.statistic,"p":r.p_value,"better":r.better,"sig":r.significant_at_05})
r = diebold_mariano(P.pm25, P.lgbm, P.pooled, truncation_lag=nw)
rows.append({"fold":"POOLED","n":r.n,"rmse_lgbm":regression_metrics(P.pm25,P.lgbm).rmse,
             "rmse_cams":regression_metrics(P.pm25,P.pooled).rmse,
             "dm":r.statistic,"p":r.p_value,"better":r.better,"sig":r.significant_at_05})
D = pd.DataFrame(rows); D.to_csv(OUT/"phase6_dm_tests.csv", index=False)
...
def fam(f):
    if f in SPATIAL_COLS: return "spatial_neighbour"
    if "valid_px" in f: return "satellite_missingness"
    if any(k in f for k in ("aod","aerosol","no2","so2","co_col")): return "satellite"
    if "cams" in f: return "cams_forecast"
    if any(k in f for k in ("doy","month","dow","heating")): return "calendar"
...
tot = famtab.sum()
for k,v in famtab.items(): print(f"  {k:24s} {v:8.3f}   {100*v/tot:5.1f}%")
...
for k,v in top.items(): print(f"    {k:34s} {v:7.3f}")
print(f"\ntables -> {OUT}")
