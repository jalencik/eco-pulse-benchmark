...
**Figures that need the panel.** Ten of the figures substituted into this document derive
from the hourly panel or its QC output, neither of which is redistributed: the
duplicate-detection shares, the shutdown-date station count and Astana's completeness. On a
clone without the panel the extractor carries those ten forward from the committed extraction
and says so in its output; every other figure is recomputed from the deposited tables.
